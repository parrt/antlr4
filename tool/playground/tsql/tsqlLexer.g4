lexer grammar tsqlLexer; //had to capitalize the L to use TestRig
...
//
/*
SQ_LITERAL	:	'N'? '\''  ( {_input.LA(1) == '\'' && _input.LA(2) == '\''}? '\'' '\'' | ~'\'' )*
					('\''|EOF)	;	// TODO: When debugging finished call "Unterminated string error"
*/
...
//
DQ_LITERAL	:	'N'? '"' ( ( ( {_input.LA(1) == '"' && _input.LA(2) == '"'}? '"' '"' | ~'"' ) *))  
					('"'|EOF)	; // TODO: When debugging finished call "Unterminated string error"
...
fragment	ML_COMFRAG
		:
			'/*' ( 
					...
					//
					{(_input.LA(1)== '/' && _input.LA(2) == '*')}? ML_COMFRAG 
					| .
				  )*?
					'*/'
		;
...
// were missing:
//DECIMAL : 'DECIMAL' ;
// SYS : 'SYS' ; // DISALLOW AS I don't see sys.tcp_endpoints etc... allowed so make SYS an ID
//SID : 'SID' ;
ACCESS : 'ACCESS' ;
...
CONCAT_NULL_YIELDS_KNULL : 'CONCAT_NULL_YIELDS_KNULL' ;
...
NUMERIC_ROUNDABOUT : 'NUMERIC_ROUNDABOUT' ;
...
// Although it would have been much more sensible to create a formal currency introducer
